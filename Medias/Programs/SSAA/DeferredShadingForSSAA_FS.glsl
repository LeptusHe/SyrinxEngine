...
void main()
{
    ...
    vec3 SceneColor = texture(uSceneColorTex, _TexCoord).rgb;
    ...
    vec3 CurrentColor = Color / vec3(8.0f);
    _outFragColor = vec4(SceneColor + CurrentColor, 1.0f);
}
...
vec3 shadingWithUnrealModelForImageBasedLight(vec3 albedo, float metallic, float roughness, vec3 normalW, vec3 viewDirW)
{
    ...
    vec3 PrefilteredColor = textureLod(uPrefilteredMap, ReflectDirW, 0).xyz;
