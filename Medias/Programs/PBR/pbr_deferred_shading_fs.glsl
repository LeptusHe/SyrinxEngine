...
void main()
{
    vec3 PositionW = texture(uGBufferPosition, _TexCoord).xyz;
    vec3 NormalW = normalize(texture(uGBufferNormal, _TexCoord).xyz);
    vec3 Albedo = texture(uGBufferAlbedo, _TexCoord).xyz;
    float Metallic = texture(uGBufferMetallic, _TexCoord).x;
    float Roughness = texture(uGBufferRoughness, _TexCoord).x;
    ...
    vec3 Ambient = vec3(0.03f) * Albedo * uAO;
