...
public:
    explicit ParametricCurve(float lambda) : m_xFitting(lambda), m_yFitting(lambda) {}

    void Solve(const std::vector<Eigen::Vector2d>& points, ParametrizationMethod method)
    {
        auto parametrizationMethod = GetParametrizationMethod(method);
        m_paramsT = parametrizationMethod->Parametric(points);
