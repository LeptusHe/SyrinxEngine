...
int main(int argc, char *argv[])
{
    ...
    auto renderWindow = displayDevice.createWindow("Normal Map Sample", WINDOW_WIDTH, WINDOW_HEIGHT);
    ...
    while (renderWindow->isOpen()) {
        ...
        for (const auto mesh : meshList) {
            ...
            auto albedoMap = material->getMaterialParameter("uAlbedoMap");
            auto albedoMapValue = albedoMap->getValue();
            auto albedoTexture = std::get<Syrinx::TextureValue>(albedoMapValue);
            glBindTextureUnit(albedoTexture.textureUnit, albedoTexture.texture->getHandle());
            fragmentProgram->updateParameter("uAlbedoMap", static_cast<int>(albedoTexture.textureUnit));

            auto normalMap = material->getMaterialParameter("uNormalMap");
            auto normalMapValue = normalMap->getValue();
            auto normalTexture = std::get<Syrinx::TextureValue>(normalMapValue);
            glBindTextureUnit(normalTexture.textureUnit, normalTexture.texture->getHandle());
            fragmentProgram->updateParameter("uNormalMap", static_cast<int>(normalTexture.textureUnit));
