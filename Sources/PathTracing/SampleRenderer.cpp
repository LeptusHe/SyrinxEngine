...
namespace osc {
  ...
  void SampleRenderer::setCamera(const Camera &camera)
  {
	...
	std::cout << "camera pos: " << camera.from.x << ", " << camera.from.y << ", " << camera.from.z << std::endl;
  ...
  void SampleRenderer::resize(const vec2i &newSize)
  {
    ...
    colorBuffer.resize(newSize.x * newSize.y * 4 * sizeof(float));
  ...
  void SampleRenderer::downloadPixels(float pixels[])
  {
	  colorBuffer.download(pixels, launchParams.frame.size.x * launchParams.frame.size.y * 4);
  }
