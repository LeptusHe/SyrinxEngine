...
namespace osc {
  ...
  struct CameraFrameManip {
    ...
    virtual void move(const float step) = 0;
  	void move(const vec3f step)
	{
		cameraFrame->position += step;
	}
  ...
  struct GLFCameraWindow : public GLFWindow {
    ...
    virtual void key(int key, int mods) override
    {
	  ...
	  case 'A':
	  {
		  vec3f step = cameraFrame.motionSpeed *  -cameraFrame.frame.vx;
		  cameraFrameManip->move(step);
			  break;
	  }
	  ...
	  case 'W':
		  {
		  vec3f step = cameraFrame.motionSpeed * cameraFrame.frame.vz;
		  cameraFrameManip->move(step);
		  break;
		  }
    ...
    virtual void mouseMotion(const vec2i &newPos) override
    {
	  ...
	  vec2f diff = 0.05f * vec2f(newPos - lastMousePos);
	  cameraFrameManip->rotate(diff.x, diff.y);
