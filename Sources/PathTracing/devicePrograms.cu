...
//------------------------------------------------------------------------------
  ...
  extern "C" __global__ void __raygen__renderFrame()
  {
	...
	float* buffer = optixLaunchParams.frame.colorBuffer;
  	
	const float prevR = buffer[index + 0];
	const float prevG = buffer[index + 1];
	const float prevB = buffer[index + 2];
  	
	const float r = (pixelColor.x + prevR * sampleCount) / (sampleCount + 1);
	const float g = (pixelColor.y + prevG * sampleCount) / (sampleCount + 1);
	const float b = (pixelColor.z + prevB * sampleCount) / (sampleCount + 1);
