...
ShaderFileIncluder::ShaderFileIncluder(const std::vector<std::string>& searchPathList)
{
...
void ShaderFileIncluder::addSearchPath(const std::string& searchPath)
{
    mFileManager.addSearchPath(searchPath);
}


shaderc_include_result* ShaderFileIncluder::GetInclude(const char *requestedSource, shaderc_include_type includeType, const char *requestingSource, size_t includeDepth)
{
...
void ShaderFileIncluder::ReleaseInclude(shaderc_include_result *includeResult)
{
